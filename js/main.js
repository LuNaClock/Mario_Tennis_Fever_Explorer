...
const translations = {
  ja: {
    ...
    accordion: { special: "特殊能力", gameText: "ゲーム内テキスト", details: "全項目を見る", video: "動画で確認する" },
  ...
  en: {
    ...
    accordion: { special: "Special", gameText: "In-game text", details: "Show all", video: "Watch video" },
...
function createRacketCard(racket) {
  ...
  const gameTextBody = document.createElement("div");
  gameTextBody.className = "racket-game-text";

  const gameText = document.createElement("p");
  gameText.textContent = localizeValue(racket.text);
  gameTextBody.append(gameText, createRacketVideoAccordion(racket));

  if (mobileView) {
    const detailsBody = document.createElement("div");
    detailsBody.className = "card-details";
    detailsBody.append(effect, createAccordion(t("accordion.gameText"), gameTextBody));
  ...
  const text = createAccordion(t("accordion.gameText"), gameTextBody);
